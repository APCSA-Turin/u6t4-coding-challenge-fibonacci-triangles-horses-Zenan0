...
public class HorseBarn {
    ...
    public void consolidate() {
        Horse[] tempStalls = new Horse[stalls.length];
        for (int i = 0; i < stalls.length; i++) {
            tempStalls[i] = stalls[i];
        }
        stalls = new Horse[tempStalls.length];
        int stallsIdx = 0;
        for (int i = 0; i < tempStalls.length; i++) {
            if (tempStalls[i] != null) {
                stalls[stallsIdx] = tempStalls[i];
                stallsIdx++;
            }
        }
    }
